...
int main(int argc, char **argv) {
    if (setresgid(0, 0, 0) != 0 || setresuid(0, 0, 0) != 0) {
        fprintf(stderr, "fedora-touchid-pam: failed to become root: %s\n", strerror(errno));
        return 126;
    }
    ...
    for (size_t i = 0; names[i] != NULL; i++) {
        ...
        if (pair == NULL) {
            fprintf(stderr, "fedora-touchid-pam: out of memory\n");
            return 127;
        }
        envp[env_index++] = pair;
    }
    ...
    child_argv[arg_index++] = "/usr/local/libexec/fedora-touchid-pam.py";
    ...
    fprintf(stderr, "fedora-touchid-pam: exec failed: %s\n", strerror(errno));
    return 127;
}
